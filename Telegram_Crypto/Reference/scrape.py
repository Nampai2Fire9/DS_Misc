...
def pdf_extract_pages(file_name, start_page, end_page):
    ...
    # Extract ALL pages instead of a subset 
def pdf_extract_all_pages(file_name):
    text_list = []
    with pdfplumber.open(file_name) as pdf:
...
def text_match_str2list(regex, text_string):
    new_re = re.compile(regex, flags=re.DOTALL)
    matches = new_re.finditer(text_string)
    match_list = []
    for match in matches:
...
def split_lists_newdf(df, field, col_names, split_val):
    ...
    #  --- Handle rows within tables that have been separated by tabula -------- 
        # Create dataframe combining target rows and replacing nan's with blank spaces. 
def row_str_repnan(df, start_row, end_row):
    row_store_list = [] 
    for i in np.arange(start_row, end_row):
        string_row = [str(x) for x in df.iloc[i]]
        row_store_list.append(['' if x =='nan' else x for x in string_row])
    return row_store_list

        # Combine rows from row_str_repnan into one row 
def combine_rowlists(row_store_list):
    new_row1 = row_store_list[0]  # initialize row
    new_row = [] 
    for i, item in enumerate(new_row1):
        for j in np.arange(1, len(row_store_list)):
            item = item + ' ' + row_store_list[j][i]
        item = item.strip(' ')     # get rid of leading and trailing spaces
        item = item.replace('  ', ' ')  # replace double spaces with single. 
        new_row.append(item)
    return new_row

    # create function to concat row into middle of df
        # Input: original dataframe, new reformatted row, start_row and end_row of original dataframe
def concat_row_middf(df, new_rowlist, start_row, end_row):
    df_stage = pd.DataFrame(df.iloc[0:start_row])
    df_stage.loc[-1] = new_rowlist
    df_stage = df_stage.reset_index(drop=True)
    df_stage = pd.concat([df_stage, df.iloc[end_row:]]).reset_index(drop=True)
    return df_stage
...
# Version of def_1st_tbl_indent (pdf scrape -> first dataframe) factoring in multiple regex for terms 
    # primary change from def_1st_tbl is using 'regex_list' argument instead of regex 
def def_1st_tbl_indent_regexplus(file_name, start_page, end_page, regex_list, roman_regex_list, letter_regex_list):
    doc_pages = pdf_extract_pages(file_name, start_page, end_page)
    doc_str = combine_strings(doc_pages) 
    
    for item in regex_list:
        reg_match = re.search(item, doc_str)
        if reg_match is not None:
            def_locs = text_match_2list(item, doc_str)
            def_descr = list_betweenloc_to_string_bounded(def_locs, doc_str, len(doc_str))
            df_defs = pd.DataFrame([doc_str[x[0]+1:x[1]-2] for x in def_locs])
            df_defs['description'] = def_descr
            df_defs.columns = ['def_name','def_description']
            ...
            def_names = []
            for i,page in enumerate(doc_pages):
                page_num = i + start_page + 1 # page number as dictated by TOC (not file). add one page as pdfplumber starts from 0
                ranges = text_match_2list(item, page)       # grab list of terms on the page      
                for range in ranges:
                    def_names.append([page[range[0]+1:range[1]-2], page_num])
            def_name_pages = pd.DataFrame(def_names, columns=['def_name','page_num'])
            # print(def_name_pages)
            df_defs = pd.merge(df_defs, def_name_pages, on='def_name', how='left')
...
# similar to def_1st_tbl_indent_regexplus but slips pdfplumber import and takes doc_pages and doc_str as input 
    # Input document page and combined string
def def_1st_tbl_indent_regexplus_noImport(doc_pages, doc_str, regex_list, 
                                          roman_regex_list, letter_regex_list,
                                          start_page = 1):
    for item in regex_list:
        reg_match = re.search(item, doc_str)
        if reg_match is not None:
...
# Version of def_pop_firstdict, but outputs list instead of dictionary
# Convert definition dataframe with first bullet layer to list 
    # Input: dataframe with definitions and first bullet layer broken out 
    # Output: List with definitions, bullets, and page numbers 
def def_pop_firstlist(df_def_comb):
    def_list = []
    for i, item in enumerate(df_def_comb['def_description']):
        key_vals = {}   # holds definition value
        sub_dict = {}   # holds items within definition value
        roman_loc1 = df_def_comb.iloc[i]['roman_bullets_loc']  # will re-use for rest of loop 
        letter_loc1 = df_def_comb.iloc[i]['letter_bullets_loc']
        key_vals['term'] = df_def_comb.iloc[i]['def_name']
        key_vals['page_num'] = df_def_comb.iloc[i]['page_num']
        key_vals['systemTerm'] = df_def_comb.iloc[i]['systemTerm']
        key_vals['definition'] = item
        ...
        # Definitions with bullets: lowercase Roman comes first. 
            ...
            def_list.append(5000)
            # def_dict[key_name] = 5000
        # def_dict[key_name] = key_vals
        def_list.append(key_vals)
    return def_list

# Create Function to return last file and document pages 
    # Input: takes in doc_pages and list of known patterns for bottom page numbers 
    # Output: Dictionary with file and document max pages
def lastFileDocPageIndentureSec(doc_pages, known_patterns):
# doc_pages = doc_pages_dict[file_name_list[0]]
    file_length = len(doc_pages)
    iter_var = file_length -1 
    file_pg_list = []
    doc_pg_list = [] 
    end_string_list = []
    maxpg_dict = {}

    while iter_var > file_length/3:
        page_text = doc_pages[iter_var]
        if isinstance(page_text, str): 
            end_of_pg_str = page_text[-30:]   # pull last 20 letters of page 
            for pattern in known_patterns:
                pg_match = re.search(pattern[0], end_of_pg_str)  
                # print(end_of_pg_str, "|", pg_match.group(pattern[1]))
                # print(end_of_pg_str)
                if pg_match is not None:
                    # print(iter_var, "|", pg_match)
                    # print(end_of_pg_str, "|", pg_match.group(pattern[1]))
                    end_string_list.append(end_of_pg_str)
                    file_pg_list.append(iter_var)  # pulls integer
                    doc_pg_list.append(pg_match.group(pattern[1]))  # pulls string     
        else:
            # print(iter_var, 'not string! : |', type(page_text))
            pass
        iter_var = iter_var -1 

    df_docPgStr = pd.DataFrame()
    df_docPgStr['file_pg'] = file_pg_list
    df_docPgStr['doc_pg'] = doc_pg_list
    df_docPgStr['doc_pg_int'] = [int(x) for x in doc_pg_list]
    df_docPgStr['eop_str'] = end_string_list
    # remove pages where doc page are greater than file size. 

    df_docPgStr = df_docPgStr[df_docPgStr['doc_pg_int'] <= file_length-1]
    max_file_pg = df_docPgStr[df_docPgStr['doc_pg_int'] == df_docPgStr['doc_pg_int'].max()]['file_pg'].iloc[0]
    max_doc_pg = df_docPgStr['doc_pg_int'].max()

    maxpg_dict['max_file_pg'] = max_file_pg
    maxpg_dict['max_doc_pg'] = max_doc_pg
    maxpg_dict['doc_eodstr_table'] = df_docPgStr
    return maxpg_dict